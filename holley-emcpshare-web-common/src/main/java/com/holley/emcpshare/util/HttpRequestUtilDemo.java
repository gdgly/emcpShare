...
public class HttpRequestUtilDemo {
    ...
    private static String       operatorSecret = "sODk0vBmxihsuerE";                         // 商户密钥
    private static String       dataSecret     = "1W6StAE4Gkl2cg7B";                         // 消息密钥
    private static String       dataSecretIV   = "4ztPS8B4bemRnWRm";                         // 消息密钥初始化向量
    private static String       sigSecret      = "vhR5XUeqmVuYfIE3";                         // 签名密钥
    private static int          timeout        = 10000;                                      // 超时时间ms
    private static String       oid            = "348375736";                                // 商户编码
    ...
    public static JSONObject httpTest(String actionUrl, Object param, QueryBean queryBean, String token) throws Exception {
        ...
        System.out.println("加密前：" + JSON.toJSONString(param, SerializerFeature.WriteMapNullValue));
    ...
    public static JSONObject queryAccountInfo(String token, String userId) throws Exception {
        QueryBean qb = new QueryBean();
        qb.setOperatorId(oid);
        qb.setTimeStamp(DateUtil.DateToLong14Str(new Date()));// yyyyMMddHHmmss
        qb.setSeq("0001");
        Map<String, Object> param = new HashMap<String, Object>();
